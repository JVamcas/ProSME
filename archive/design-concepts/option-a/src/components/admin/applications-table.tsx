...
const columns = helper.columns([
  ...
  helper.display({ id: "action", header: "", cell: info => <Link href={`/admin/applications/${info.row.original.id}`} className="grid size-8 place-items-center rounded-full border border-slate-200 text-slate-400 hover:border-sky hover:text-navy" aria-label={`View ${info.row.original.id}`}><ChevronRight className="size-4"/></Link> }),
]);
