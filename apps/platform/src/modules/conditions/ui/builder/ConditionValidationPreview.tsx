...
export function ConditionValidationPreview({
  ...
  return (
    <section
      ...
      className="mt-4 rounded-2xl border border-brand-navy/15 bg-brand-white p-1"
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3
          className="font-semibold text-brand-navy"
          id="condition-preview-heading text-sm"
        >
          Condition preview
        </h3>
      ...
      <p className="mt-3 text-xs leading-6 text-brand-navy" role="status">
        {preview}
      </p>
      {validation.valid ? null : (
        <ul className="mt-3 space-y-1 text-xs text-red-700" role="alert">
