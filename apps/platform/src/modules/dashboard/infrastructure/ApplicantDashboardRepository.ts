...
const dashboardSelect = sql`
  SELECT
      ...
      (
        ...
        WHERE funding_call.status IN ('SCHEDULED', 'LIVE')
          AND funding_call.opens_at <= CURRENT_TIMESTAMP
          AND funding_call.closes_at > CURRENT_TIMESTAMP
          AND EXISTS (
            SELECT 1
            FROM app_funding_call_publication_revisions publication
            WHERE publication.funding_call_id = funding_call.id
