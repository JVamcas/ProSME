...
export async function appendTaskCompletionAudit(
  transaction: RuntimeAuditTransaction,
  input: TaskCompletionAuditInput,
) {
  const completion = {
  ...
  await transaction.execute(sql`
    INSERT INTO app_workflow_audit_entries
      (actor_id, action, target_type, target_id, correlation_id,
       ...
       reason, before, after)
    VALUES (${input.actorId}::uuid, 'TASK_COMPLETED', 'WORKFLOW_TASK',
