...
export function AdminDashboard() {
  return <div className="mx-auto max-w-[1240px] p-4 sm:p-7 lg:p-8">
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[1.65fr_.75fr]">
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-sm font-bold text-navy">Applications Overview</h2><div className="mt-4 h-72"><ResponsiveContainer width="100%" height="100%"><AreaChart data={trend} margin={{top:10,right:10,left:-22,bottom:0}}><defs><linearGradient id="overviewFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#ff6f00" stopOpacity={0.28}/><stop offset="100%" stopColor="#ff6f00" stopOpacity={0}/></linearGradient></defs><CartesianGrid stroke="#edf0f3" vertical={true}/><XAxis dataKey="day" axisLine={false} tickLine={false} interval={1} tick={{fontSize:10,fill:"#94a3b8"}}/><YAxis domain={[0,30]} ticks={[0,10,20,30]} axisLine={false} tickLine={false} tick={{fontSize:10,fill:"#94a3b8"}}/><Tooltip contentStyle={{borderRadius:8,borderColor:"#e2e8f0",fontSize:11}}/><Area type="linear" dataKey="applications" stroke="#d95e00" strokeWidth={2} fill="url(#overviewFill)"/></AreaChart></ResponsiveContainer></div></section>
