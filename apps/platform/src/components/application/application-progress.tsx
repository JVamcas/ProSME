    ...
    <div className="border-b border-slate-200 bg-slate-50 px-5 py-5 sm:px-8">
      <div className="flex items-center justify-between gap-5 overflow-x-auto">
        {steps.map(({ title, icon: Icon }, index) => (
          <div
            key={title}
            className={`flex min-w-fit items-center gap-2 text-xs font-bold ${index === current ? "text-navy" : index < current ? "text-emerald-700" : "text-slate-400"}`}
          >
            <span
              className={`grid size-8 place-items-center rounded-full ${index === current ? "bg-navy text-brand-orange" : index < current ? "bg-emerald-100" : "border border-slate-200 bg-white text-brand-orange opacity-60"}`}
            >
              {index < current ? (
