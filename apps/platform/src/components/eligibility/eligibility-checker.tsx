...
export function EligibilityChecker({ rules }: { rules: EligibilityRule[] }) {
  ...
  if (complete) {
    ...
    return <div className="card overflow-hidden">
      ...
      <div className="flex flex-col gap-3 border-t border-slate-200 bg-white p-6 sm:flex-row sm:justify-between"><Button variant="ghost" onClick={reset}><RotateCcw className="size-4" />Start again</Button>{result.type !== "not-eligible" && <Button asChild variant="gold"><Link href="/portal/applications/new">Start application <ArrowRight className="size-4" /></Link></Button>}</div>
    </div>;
  }
