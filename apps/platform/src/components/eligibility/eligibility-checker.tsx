...
import { EligibilityAnswerButton } from "./eligibility-answer-button";
...
export function EligibilityChecker({ rules }: { rules: EligibilityRule[] }) {
  ...
  const result = useMemo(() => {
    const hardFailures = rules.filter(
      (r) => r.hardStop && answers[r.id] === "no",
    );
    const actions = rules.filter((r) => !r.hardStop && answers[r.id] === "no");
    if (hardFailures.length)
      return { type: "not-eligible" as const, items: hardFailures };
    ...
    return (
      <div className="card p-8">
        <h2 className="text-xl font-bold text-navy">
          Eligibility checker unavailable
        </h2>
        <p className="mt-3 text-sm text-slate-600">
          The screening questions have not been published yet. Please check the
          current funding criteria or contact the programme team.
        </p>
      </div>
    );
  ...
  if (complete) {
    ...
    return (
      <div className="card overflow-hidden">
        <div
          className={`p-8 sm:p-10 ${eligible ? "bg-emerald-50" : action ? "bg-amber-50" : "bg-red-50"}`}
        >
          <div
            className={`grid size-14 place-items-center rounded-full ${eligible ? "bg-emerald-600" : action ? "bg-amber-500" : "bg-red-600"} text-white`}
          >
            {eligible ? <CheckCircle2 /> : <AlertCircle />}
          </div>
          <p className="eyebrow mt-7">Your result</p>
          <h2 className="display mt-2 text-4xl font-semibold text-navy">
            {eligible
          ...
          <p className="mt-4 max-w-2xl leading-7 text-slate-600">
            {eligible
              ? "Based on your responses, you meet the initial requirements. Final eligibility is subject to document verification and assessment."
              : action
                ? "You meet the core requirements, but should resolve the items below before submitting a complete application."
                : "One or more core programme requirements are not currently met. Review the items below before proceeding."}
          </p>
        </div>
        {result.items.length > 0 && (
          <div className="border-t border-slate-200 p-8 sm:p-10">
            <h3 className="font-bold text-navy">Items to review</h3>
  ...
  return (
    <div className="card overflow-hidden">
      <div className="border-b border-slate-200 px-6 py-5 sm:px-9">
        <div className="flex items-center justify-between text-xs font-bold">
          <span className="text-navy">
            Question {index + 1} of {rules.length}
          </span>
      ...
      <div className="p-7 sm:p-10">
        <p className="eyebrow">Initial eligibility</p>
        <h2 className="display mt-3 text-3xl font-semibold leading-tight text-navy sm:text-4xl">
          {rule.question}
        </h2>
        ...
        <div className="mt-9 grid gap-3 sm:grid-cols-2">
          <EligibilityAnswerButton answer="yes" onSelect={answer} />
          <EligibilityAnswerButton answer="no" onSelect={answer} />
        </div>
      </div>
